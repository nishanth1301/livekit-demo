...
function warnAboutMissingStyles(el) {
  ...
  ((_b = process == null ? void 0 : process.env) == null ? void 0 : _b.NODE_ENV) === "development")) {
    ...
    if (target && !getComputedStyle(target).getPropertyValue("--lk-has-imported-styles")) {
      import_components_core2.log.warn(
...
function useConnectionState(room) {
  ...
  const observable = React11.useMemo(() => (0, import_components_core4.connectionStateObserver)(r), [r]);
...
function useChat(options) {
  ...
  const setup = React12.useMemo(
    () => (0, import_components_core5.setupChat)(room, options),
...
function useConnectionQualityIndicator(options = {}) {
  ...
  const { className, connectionQualityObserver } = React13.useMemo(
    () => (0, import_components_core6.setupConnectionQualityIndicator)(p),
    [p]
  );
  const quality = useObservableState(connectionQualityObserver, import_livekit_client2.ConnectionQuality.Unknown);
...
function useDisconnectButton(props) {
  ...
  const buttonProps = React14.useMemo(() => {
    ...
    const mergedProps = mergeProps(props, {
      ...
      disabled: connectionState === import_livekit_client3.ConnectionState.Disconnected
    });
    return mergedProps;
...
function useFacingMode(trackReference) {
  if (trackReference.publication instanceof import_livekit_client4.LocalTrackPublication) {
    const localTrack = trackReference.publication.track;
    if (localTrack) {
      const { facingMode } = (0, import_livekit_client4.facingModeFromLocalTrack)(localTrack);
      return facingMode;
    }
  }
  return "undefined";
...
function useMediaDeviceSelect({
  ...
  const deviceObserver = React20.useMemo(
    () => (0, import_components_core13.createMediaDeviceObserver)(kind, onError, requestPermissions),
...
function useMediaDevices({
  ...
  const deviceObserver = React21.useMemo(
    () => (0, import_components_core14.createMediaDeviceObserver)(kind, onError),
    [kind, onError]
  );
  const devices = useObservableState(deviceObserver, []);
  return devices;
...
function useVisualStableUpdate(trackReferences, maxItemsOnPage, options = {}) {
  ...
  const sortedTrackRefs = typeof options.customSortFunction === "function" ? options.customSortFunction(trackReferences) : (0, import_components_core15.sortTrackReferences)(trackReferences);
  let updatedTrackRefs = [...sortedTrackRefs];
  if (layoutChanged === false) {
    try {
      updatedTrackRefs = (0, import_components_core15.updatePages)(lastTrackRefs.current, sortedTrackRefs, maxItemsOnPage);
    } catch (error) {
      import_components_core15.log.error("Error while running updatePages(): ", error);
    }
  }
  if (layoutChanged) {
...
function useParticipantTile({
  ...
  const mergedProps = React24.useMemo(() => {
    const { className } = (0, import_components_core16.setupParticipantTile)();
...
function useStartAudio({ room, props }) {
  ...
  const { className, roomAudioPlaybackAllowedObservable, handleStartAudioPlayback } = React26.useMemo(
    () => (0, import_components_core17.setupStartAudio)(),
    []
  );
...
function useStartVideo({ room, props }) {
  ...
  const { className, roomVideoPlaybackAllowedObservable, handleStartVideoPlayback } = React27.useMemo(
    () => (0, import_components_core18.setupStartVideo)(),
    []
  );
...
function useChatToggle({ props }) {
  ...
  const { className } = React29.useMemo(() => (0, import_components_core19.setupChatToggle)(), []);
...
function useTrackMutedIndicator(trackRef) {
  ...
  const { className, mediaMutedObserver } = React30.useMemo(
    () => (0, import_components_core20.setupTrackMutedIndicator)(trackReference),
    [(0, import_components_core20.getTrackReferenceId)(trackReference)]
  );
...
function useTrackToggle(_a) {
  ...
  const { toggle, className, pendingObserver, enabledObserver } = React31.useMemo(
    () => room ? (0, import_components_core21.setupMediaToggle)(source, room, captureOptions, publishOptions, onDeviceError) : (0, import_components_core21.setupManualToggle)(),
  ...
  React31.useEffect(() => {
    if (initialState !== void 0) {
      import_components_core21.log.debug("forcing initial toggle state", source, initialState);
      toggle(initialState);
    }
  }, []);
...
function useTracks(sources = [
  ...
  const sources_ = React32.useMemo(() => {
    return sources.map((s) => (0, import_components_core22.isSourceWitOptions)(s) ? s.source : s);
  }, [JSON.stringify(sources)]);
  React32.useEffect(() => {
    const subscription = (0, import_components_core22.trackReferencesObservable)(room, sources_, {
      additionalRoomEvents: options.updateOnlyOn,
      onlySubscribed: options.onlySubscribed
    }).subscribe(({ trackReferences: trackReferences2, participants: participants2 }) => {
      import_components_core22.log.debug("setting track bundles", trackReferences2, participants2);
  ...
  const maybeTrackReferences = React32.useMemo(() => {
    if ((0, import_components_core22.isSourcesWithOptions)(sources)) {
      ...
      participants.forEach((participant) => {
        ...
        if (requirePlaceholder.has(participant.identity)) {
          ...
          sourcesToAddPlaceholder.forEach((placeholderSource) => {
            ...
            import_components_core22.log.debug(
...
function usePersistentUserChoices(options = {}) {
  ...
  const [userChoices, setSettings] = React33.useState(
    (0, import_components_core23.loadUserChoices)(options.defaults, (_a = options.preventLoad) != null ? _a : false)
  );
  ...
  React33.useEffect(() => {
    var _a2;
    (0, import_components_core23.saveUserChoices)(userChoices, (_a2 = options.preventSave) != null ? _a2 : false);
...
function useIsEncrypted(participant, options = {}) {
  const p = useEnsureParticipant(participant);
  const room = useEnsureRoom(options.room);
  const observer = React34.useMemo(() => (0, import_components_core24.encryptionStatusObservable)(room, p), [room, p]);
...
var ParticipantName = /* @__PURE__ */ React62.forwardRef(
  function ParticipantName2(_a, ref) {
    ...
    const { className, infoObserver } = React62.useMemo(() => {
      return (0, import_components_core26.setupParticipantName)(p);
    }, [p]);
...
function useMediaTrackBySourceOrName(observerOptions, options = {}) {
  var _a;
  const [publication, setPublication] = React65.useState((0, import_components_core28.getTrackByIdentifier)(observerOptions));
  ...
  const { className, trackObserver } = React65.useMemo(() => {
    return (0, import_components_core28.setupMediaTrack)(observerOptions);
  }, [
    (_a = observerOptions.participant.sid) != null ? _a : observerOptions.participant.identity,
    observerOptions.source,
    (0, import_components_core27.isTrackReference)(observerOptions) && observerOptions.publication.trackSid
  ]);
  React65.useEffect(() => {
    const subscription = trackObserver.subscribe((publication2) => {
      import_components_core28.log.debug("update track", publication2);
  ...
  React65.useEffect(() => {
    ...
    if (track) {
      ...
      if (((_a2 = options.element) == null ? void 0 : _a2.current) && !((0, import_components_core28.isLocal)(observerOptions.participant) && (track == null ? void 0 : track.kind) === "audio")) {
...
var VideoTrack = /* @__PURE__ */ React66.forwardRef(
  function VideoTrack2(_a, ref) {
    ...
    React66.useEffect(() => {
      if (manageSubscription && trackReference.publication instanceof import_livekit_client11.RemoteTrackPublication && (debouncedIntersectionEntry == null ? void 0 : debouncedIntersectionEntry.isIntersecting) === false && (intersectionEntry == null ? void 0 : intersectionEntry.isIntersecting) === false) {
        trackReference.publication.setSubscribed(false);
      }
    }, [debouncedIntersectionEntry, trackReference, manageSubscription]);
    React66.useEffect(() => {
      if (manageSubscription && trackReference.publication instanceof import_livekit_client11.RemoteTrackPublication && (intersectionEntry == null ? void 0 : intersectionEntry.isIntersecting) === true) {
  ...
  function AudioTrack2(_a, ref) {
    ...
    React67.useEffect(() => {
      ...
      if (pub instanceof import_livekit_client12.RemoteTrackPublication) {
        ...
        import_components_core29.log.warn("Can only call setEnabled on remote track publications.");
      }
    }, [muted, pub, track]);
...
var ParticipantTile = /* @__PURE__ */ React68.forwardRef(
  function ParticipantTile2(_a, ref) {
    ...
    const handleSubscribe = React68.useCallback(
      (subscribed) => {
        if (trackReference.source && !subscribed && layoutContext && layoutContext.pin.dispatch && (0, import_components_core30.isTrackReferencePinned)(trackReference, layoutContext.pin.state)) {
          layoutContext.pin.dispatch({ msg: "clear_pin" });
        }
      },
      [trackReference, layoutContext]
    );
    return /* @__PURE__ */ React68.createElement("div", __spreadValues({ ref, style: { position: "relative" } }, elementProps), /* @__PURE__ */ React68.createElement(TrackRefContextIfNeeded, { trackRef: trackReference }, /* @__PURE__ */ React68.createElement(ParticipantContextIfNeeded, { participant: trackReference.participant }, children != null ? children : /* @__PURE__ */ React68.createElement(React68.Fragment, null, (0, import_components_core30.isTrackReference)(trackReference) && (((_b2 = trackReference.publication) == null ? void 0 : _b2.kind) === "video" || trackReference.source === import_livekit_client13.Track.Source.Camera || trackReference.source === import_livekit_client13.Track.Source.ScreenShare) ? /* @__PURE__ */ React68.createElement(
      VideoTrack,
      {
        trackRef: trackReference,
  ...
  return /* @__PURE__ */ React70.createElement(React70.Fragment, null, tracks.map((trackReference) => {
    return /* @__PURE__ */ React70.createElement(
      TrackRefContext.Provider,
      {
        ...
        key: (0, import_components_core31.getTrackReferenceId)(trackReference)
      },
...
function PaginationControl({
  ...
  React71.useEffect(() => {
    let subscription;
    if (connectedElement) {
      subscription = (0, import_components_core32.createInteractingObservable)(connectedElement.current, 2e3).subscribe(
        setInteractive
      );
    }
    return () => {
...
function CarouselLayout(_a) {
  ...
  const scrollBarWidth = (0, import_components_core33.getScrollBarWidth)();
...
function LayoutContextProvider({
  ...
  React75.useEffect(() => {
    import_components_core34.log.debug("PinState Updated", { state: layoutContextValue.pin.state });
  ...
  React75.useEffect(() => {
    import_components_core34.log.debug("Widget Updated", { widgetState: layoutContextValue.widget.state });
...
function RoomAudioRenderer({ volume, muted }) {
  ...
  ).filter((ref) => !(0, import_components_core35.isLocal)(ref.participant) && ref.publication.kind === import_livekit_client14.Track.Kind.Audio);
  return /* @__PURE__ */ React77.createElement("div", { style: { display: "none" } }, tracks.map((trackRef) => /* @__PURE__ */ React77.createElement(
    AudioTrack,
    {
      key: (0, import_components_core35.getTrackReferenceId)(trackRef),
      trackRef,
      volume,
      muted
    }
...
function MediaDeviceMenu(_a) {
  ...
  const handleActiveDeviceChange = (kind2, deviceId) => {
    import_components_core38.log.debug("handle device change");
    setIsOpen(false);
...
function usePreviewTracks(options, onError) {
  ...
  const trackLock = React84.useMemo(() => new import_livekit_client16.Mutex(), []);
  React84.useEffect(() => {
    ...
    trackLock.lock().then((unlock) => __async(this, null, function* () {
      try {
        if (options.audio || options.video) {
          localTracks = yield (0, import_livekit_client16.createLocalTracks)(options);
          if (needsCleanup) {
        ...
        if (onError && e instanceof Error) {
          ...
          import_components_core39.log.error(e);
        }
      } finally {
        unlock();
      }
...
function usePreviewDevice(enabled, deviceId, kind) {
  ...
  React84.useEffect(() => {
    if (enabled && !localTrack && !deviceError && !isCreatingTrack) {
      import_components_core39.log.debug("creating track", kind);
  ...
  React84.useEffect(() => {
    ...
    if (!enabled) {
      import_components_core39.log.debug(`muting ${kind} track`);
      localTrack.mute().then(() => import_components_core39.log.debug(localTrack.mediaStreamTrack));
      ...
      import_components_core39.log.debug(`switching ${kind} device from`, prevDeviceId.current, selectedDevice.deviceId);
      switchDevice(localTrack, selectedDevice.deviceId);
    } else {
      import_components_core39.log.debug(`unmuting local ${kind} track`);
      localTrack.unmute();
    }
  ...
  React84.useEffect(() => {
    return () => {
      if (localTrack) {
        import_components_core39.log.debug(`stopping local ${kind} track`);
        localTrack.stop();
        localTrack.mute();
      }
    };
...
function PreJoin(_a) {
  ...
  const [userChoices, setUserChoices] = React84.useState(import_components_core40.defaultUserChoices);
  ...
  function handleSubmit(event) {
    ...
    if (handleValidation(userChoices)) {
      ...
      import_components_core39.log.warn("Validation failed with: ", userChoices);
    }
  }
...
function ControlBar(_a) {
  ...
  const browserSupportsScreenSharing = (0, import_components_core41.supportsScreenSharing)();
  ...
  ))), visibleControls.camera && /* @__PURE__ */ React88.createElement("div", { className: "lk-button-group" }, /* @__PURE__ */ React88.createElement(TrackToggle, { source: import_livekit_client17.Track.Source.Camera, showIcon, onChange: cameraOnChange }, showText && "Camera"), /* @__PURE__ */ React88.createElement("div", { className: "lk-button-group-menu" }, /* @__PURE__ */ React88.createElement(
    MediaDeviceMenu,
    {
      kind: "videoinput",
...
function VideoConference(_a) {
  ...
  const tracks = useTracks(
    [
      { source: import_livekit_client18.Track.Source.Camera, withPlaceholder: true },
      { source: import_livekit_client18.Track.Source.ScreenShare, withPlaceholder: false }
    ],
  ...
  React89.useEffect(() => {
    ...
    if (screenShareTracks.some((track) => track.publication.isSubscribed) && lastAutoFocusedScreenShareTrack.current === null) {
      import_components_core42.log.debug("Auto set screen share focus:", { newScreenShareTrack: screenShareTracks[0] });
      ...
      import_components_core42.log.debug("Auto clearing screen share focus.");
      (_d = (_c = layoutContext.pin).dispatch) == null ? void 0 : _d.call(_c, { msg: "clear_pin" });
      lastAutoFocusedScreenShareTrack.current = null;
    }
    if (focusTrack && !(0, import_components_core42.isTrackReference)(focusTrack)) {
      const updatedFocusTrack = tracks.find(
        (tr) => tr.participant.identity === focusTrack.participant.identity && tr.source === focusTrack.source
      );
      if (updatedFocusTrack !== focusTrack && (0, import_components_core42.isTrackReference)(updatedFocusTrack)) {
  ...
  useWarnAboutMissingStyles();
  return /* @__PURE__ */ React89.createElement("div", __spreadValues({ className: "lk-video-conference" }, props), (0, import_components_core42.isWeb)() && /* @__PURE__ */ React89.createElement(
...
function AudioConference(_a) {
  ...
  const audioTracks = useTracks([import_livekit_client19.Track.Source.Microphone]);
  useWarnAboutMissingStyles();
